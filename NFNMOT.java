 ...
 */
public class NFNMOT extends Filter
...
////        // compute Value Distance Metric matrices for nominal features
    ...
    public Instances process(int maxcount, int min, int minIndex, int nearestNeighbors, Instances orignalset, Instances sample) {
        ...
        int os = 0, pos = -1;
        int totalcount = 0, ptotalcount = -1, end = 0, pmaxcount = maxcount, oscount = -1;
            ...
            for (int i = 0; i < sample.numInstances(); i++) {
                ...
                while (nearestNeighbors > 2) {
                    ...
                    trail--;
                    {
                        ...
                        int part = nearestNeighbors / 2;
                        j = 0;
                        while (entryIterator.hasNext() && j < part) {
                            nnArray[j] = (Instance) ((Object[]) entryIterator.next())[1];
                            j++;
                        }
                        int n = distanceToInstance.size() - 1;
                        for (int k = 0; k < part; k++) {
                            nnArray[j] = (Instance) ((Object[]) distanceToInstance.get(n))[1];
                            j++;
                            n--;
                        }
                        if (nearestNeighbors % 2 == 1) {
                            nnArray[j] = (Instance) ((Object[]) distanceToInstance.get(distanceToInstance.size() / 2))[1];

                        }
                        int classtype = getSampleClass(nnArray, minIndex, sample, nearestNeighbors);
    ...
    public Instance getSMOTE(int minIndex, Instance sampleInstanceI, Instance instanceI) {
        ...
        Enumeration<Attribute> attrEnum = sampleInstanceI.enumerateAttributes();
        while (attrEnum.hasMoreElements()) {
            Attribute attr = (Attribute) attrEnum.nextElement();
            if (!attr.equals(sampleInstanceI.classAttribute())) {
                if (attr.isNumeric()) {
...
//                                    }
        ...
        synthetic.setDataset(sampleInstanceI.dataset());
    ...
    public Instance getMinortyMajorritySample(Instance sample, Instance[] sampleAll, Map vdmMap, int minIndex, Instance orignalSample) {
        ...
        for (int j = 0; j < sampleAll.length; j++) {
            ...
            System.out.println(clasindex + ":" + minIndex);
    ...
    public int getSampleClass(Instance[] distanceToInstance, double minIndex, Instances sample, int localnearestNeighbors) {

        int minority = -1, majorty = -1;
        for (int i = 0; i < localnearestNeighbors; i++) {
            // val = (alInstances.get(i).size() / (os + 1));
            if (((Instance) distanceToInstance[1]).classValue() == minIndex) {
                minority = 1;
            } else {
                majorty = 1;
            }
